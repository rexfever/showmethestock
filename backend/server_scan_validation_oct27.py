...
def generate_report(scan_results: list, returns_results: list, output_file: str):
    """성과 리포트 생성"""
    print(f"\n📊 리포트 생성 중...")
    ...
    report.append("=" * 100)
    report.append("")
    report.append(f"검증 기간: 10월 마지막주 (10/27~10/31) & 11월 첫째주 (11/3~11/7)")
...
def main():
    ...
    # 1. 거래일 생성
    print("\n📅 거래일 생성")
    oct_last_week = get_trading_days("2025-10-27", "2025-10-31")
    nov_first_week = get_trading_days("2025-11-03", "2025-11-07")
    
    all_dates = oct_last_week + nov_first_week
    
    print(f"  - 10월 마지막주: {oct_last_week}")
    print(f"  - 11월 첫째주: {nov_first_week}")
    ...
    # 4. 리포트 생성
    output_file = "server_scan_validation_report.txt"
    generate_report(scan_results, returns_results, output_file)
