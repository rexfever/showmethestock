"""
ARCHIVED 상태이면서 archive_return_pct가 10% 이상인 추천 확인
"""
import sys
import os
...
def check_high_return_archived():
    """archive_return_pct가 10% 이상인 ARCHIVED 추천 확인"""
    try:
        with db_manager.get_cursor(commit=False) as cur:
            # archive_return_pct가 10% 이상인 ARCHIVED 추천 조회
            cur.execute("""
                SELECT 
                    recommendation_id,
                    ticker,
            ...
            if not rows:
                print("archive_return_pct가 10% 이상인 ARCHIVED 추천이 없습니다.")
                return
            
            print(f"총 {len(rows)}개의 ARCHIVED 추천 발견 (10% 이상)\n")
            print("=" * 120)
