"""
ARCHIVED 데이터를 정책에 맞게 재계산하여 업데이트 (종합)
1. BROKEN을 거친 경우: BROKEN 시점 스냅샷 사용 (broken_return_pct 사용)
2. REPLACED 케이스: REPLACED 전환 시점 스냅샷 사용 (status_changed_at 시점 OHLCV)
            ...
            for idx, row in enumerate(rows, 1):
                ...
                if broken_at and broken_return_pct is not None:
                    ...
                    if archive_return_pct is None:
                        ...
                        needs_update = True
                        update_reasons.append(f"BROKEN 스냅샷: {archive_return_pct} → {new_archive_return_pct}")
