"""
archive_reason이 NO_MOMENTUM이면서 archive_return_pct가 10% 이상인 추천 확인
"""
import sys
import os
...
def check_no_momentum_high_return():
    """NO_MOMENTUM이면서 10% 이상인 ARCHIVED 추천 확인"""
    try:
        with db_manager.get_cursor(commit=False) as cur:
            cur.execute("""
                ...
                AND archive_return_pct >= 10.0
                ORDER BY archive_return_pct DESC
                LIMIT 20
            """)
            ...
            if not rows:
                print("NO_MOMENTUM이면서 10% 이상인 ARCHIVED 추천이 없습니다.")
                return
            
            print(f"총 {len(rows)}개의 NO_MOMENTUM + 10% 이상 ARCHIVED 추천 발견\n")
            print("=" * 120)
            ...
            for idx, row in enumerate(rows, 1):
                ...
                print(f"    ARCHIVED 시점 수익률 (archive_return_pct): {row[9]:,.2f}%")
                ...
                if row[6]:  # broken_at이 있으면
                    ...
                    if row[7] is not None:
                        ...
                        print(f"    ⚠️  문제: BROKEN 시점에 {row[7]}%였지만, ARCHIVED 전환 시점에는 {row[9]:,.2f}%로 회복됨")
                        print(f"    ⚠️  하지만 archive_reason은 BROKEN 시점의 'NO_MOMENTUM'이 그대로 유지됨")
                else:
                    print(f"    → ACTIVE에서 직접 ARCHIVED로 전환됨")
                    print(f"    ⚠️  문제: {row[9]:,.2f}% 수익인데 'NO_MOMENTUM' 사유로 종료됨")
