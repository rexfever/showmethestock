...
def recalculate_all_archived_data():
    """ARCHIVED 데이터를 정책에 맞게 재계산하여 업데이트"""
    try:
        with db_manager.get_cursor(commit=False) as cur:
            ...
            for idx, row in enumerate(rows, 1):
                ...
                # 1. BROKEN을 거친 경우: BROKEN 시점 스냅샷 사용 (최우선)
                if broken_at and broken_return_pct is not None:
                    ...
                    if archive_return_pct is None:
                        needs_update = True
                        update_reason.append("archive_return_pct가 None")
                    elif abs(float(archive_return_pct) - new_archive_return_pct) > 0.01:
                        needs_update = True
                        update_reason.append(f"BROKEN 스냅샷 불일치: {archive_return_pct} → {new_archive_return_pct}")
                    
                    if archive_price is None or (new_archive_price and abs(float(archive_price) - new_archive_price) > 1):
                        needs_update = True
                        if "가격" not in " ".join(update_reason):
                            update_reason.append("archive_price 불일치")
                    
                    if archive_phase != new_archive_phase:
                        needs_update = True
                        if "phase" not in " ".join(update_reason):
                            update_reason.append(f"archive_phase 불일치: {archive_phase} → {new_archive_phase}")
                        ...
                        if not df.empty:
                            ...
                            if replaced_price and anchor_close:
                                ...
                                if archive_return_pct is None:
                                    ...
                                    needs_update = True
                                    update_reason.append(f"REPLACED 스냅샷 불일치: {archive_return_pct} → {new_archive_return_pct}")
                        ...
                        if not df.empty:
                            ...
                            if ttl_price and anchor_close:
                                ...
                                if archive_return_pct is None:
                                    ...
                                    needs_update = True
                                    update_reason.append(f"TTL 만료 시점 스냅샷 불일치: {archive_return_pct} → {new_archive_return_pct}")
                ...
                # 4. 손절 조건 체크: 손절 조건 만족 시 archive_reason을 NO_MOMENTUM으로 변경
                if new_archive_return_pct is not None:
                    ...
                    if new_archive_return_pct <= stop_loss_pct:
                        if new_archive_reason != 'NO_MOMENTUM':
                            ...
                            needs_update = True
                            update_reason.append(f"손절 조건 만족: {new_archive_return_pct:.2f}% <= {stop_loss_pct:.2f}%")
