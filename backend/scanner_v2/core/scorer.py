...
class Scorer:
    ...
    def determine_strategy(self, flags: Dict, score: float) -> str:
        """전략 결정 (별도 함수 사용)"""
        strategy, _, _, _ = determine_trading_strategy(flags, score)
        return strategy
    
    def _get_weights(self) -> Dict[str, float]:
        """점수 가중치 가져오기"""
        # scanner_v2 전용 config 사용
    ...
    def _calculate_risk_score(self, df: pd.DataFrame) -> Tuple[int, Dict]:
        """위험도 점수 계산 (scanner.py의 calculate_risk_score와 동일한 로직)"""
        if len(df) < 21:
            return 0, {}
        ...
        rsi_overbought = cur.RSI_TEMA > 80
        ...
        vol_spike = cur.volume > (cur.VOL_MA5 * vol_spike_threshold if pd.notna(cur.VOL_MA5) else cur.volume)
