#!/usr/bin/env python3
"""
2025년 7월부터 10월까지의 레짐 캐시 최신화 및 분석 결과 강제 업데이트
- 기존 데이터가 있어도 최신 캐시로 재분석
"""
import os
import sys
import pandas as pd
...
def analyze_and_save_regime_v4(date: str, analyzer: MarketAnalyzer, force_update: bool = True) -> bool:
    """레짐 v4 분석 및 DB 저장 (강제 업데이트)"""
    try:
        logger.info(f"레짐 v4 분석 시작: {date} (강제 업데이트)")
        ...
        if hasattr(market_condition, 'final_regime'):
            logger.info(f"✅ 레짐 v4 분석 완료: {date} -> {market_condition.final_regime} (trend: {market_condition.global_trend_score:.2f}, risk: {market_condition.global_risk_score:.2f})")
            return True
        else:
...
def main():
    """메인 함수"""
    logger.info("=" * 70)
    logger.info("2025년 7월~10월 레짐 캐시 최신화 및 강제 업데이트 시작")
    logger.info("=" * 70)
    ...
    # 2단계: 레짐 분석 및 DB 저장 (강제 업데이트)
    logger.info("\n" + "=" * 70)
    logger.info("[2단계] 레짐 v4 분석 및 DB 저장 (강제 업데이트)")
    logger.info("=" * 70)
    
    # 거래일 리스트 생성
    trading_days = get_trading_days(start_date, end_date)
    logger.info(f"총 거래일: {len(trading_days)}일")
    logger.info("⚠️ 기존 데이터가 있어도 최신 캐시로 재분석합니다.")
    ...
    for i, date in enumerate(trading_days, 1):
        try:
            ...
            # 레짐 v4 분석 및 저장 (강제 업데이트)
            if analyze_and_save_regime_v4(date, analyzer, force_update=True):
                success_count += 1
            ...
            # 진행 상황 출력
            if i % 10 == 0:
                logger.info(f"\n진행 상황: {i}/{len(trading_days)} ({success_count}개 성공, {failed_count}개 실패)")
    ...
    # 최종 결과
    logger.info("\n" + "=" * 70)
    logger.info("2025년 7월~10월 레짐 캐시 최신화 및 강제 업데이트 완료")
    logger.info("=" * 70)
    logger.info(f"총 거래일: {len(trading_days)}일")
