#!/usr/bin/env python3
"""
2025년 11월 v2 스캔 실행 및 DB 저장 (레짐 v4 사용)
"""
import os
import sys
import pandas as pd
...
def run_scan_v2_for_date(date: str, force_update: bool = False) -> bool:
    """특정 날짜에 대해 v2 스캔 실행 및 DB 저장"""
    try:
        normalized_date = normalize_date(date)
        
        # 기존 데이터 확인
        if not force_update and check_existing_scan(normalized_date, 'v2'):
            logger.info(f"이미 v2 스캔 데이터가 있음: {normalized_date}, 건너뜀")
            return True
...
def main():
    """메인 함수"""
    logger.info("=" * 70)
    logger.info("2025년 11월 v2 스캔 및 DB 저장 시작 (레짐 v4)")
    logger.info("=" * 70)
    ...
    # 11월이 아니면 11월 마지막 날로 제한
    if end_date < '20251101':
        logger.warning("현재 날짜가 11월이 아닙니다. 11월 마지막 날로 설정합니다.")
        end_date = '20251130'
    elif end_date > '20251130':
        end_date = '20251130'
    ...
    for i, date in enumerate(trading_days, 1):
        try:
            ...
            if check_existing_scan(date, 'v2'):
                logger.info(f"  기존 v2 스캔 데이터가 있음: {date}, 강제 업데이트 진행")
            
            # v2 스캔 실행 및 저장 (강제 업데이트)
            if run_scan_v2_for_date(date, force_update=True):
                success_count += 1
    ...
    logger.info("\n" + "=" * 70)
    logger.info("2025년 11월 v2 스캔 및 DB 저장 완료")
    logger.info("=" * 70)
    logger.info(f"총 거래일: {len(trading_days)}일")
