...
**이유**:
- 메모리 캐시는 프로세스의 RAM에만 저장됨
- 프로세스 종료 → 메모리 해제 → 캐시 소실
- TTL은 "프로세스 실행 중 유효 기간"이지 "프로세스 재시작 후에도 유효"가 아님
...
- 과거 날짜는 TTL이 1년이므로 **프로세스 실행 중**에는 캐시가 오래 유지됨

**단점**:
- ⚠️ **프로세스가 종료되면 캐시가 완전히 사라짐** (TTL 1년과 무관)
- 첫 번째 실행 시에는 여전히 API 호출 필요
- 프로세스 재시작 후에는 다시 API 호출 필요
...
# TTL 1년이지만 프로세스 종료 → 메모리 해제 → 캐시 완전히 소실

# 프로세스 2 (새로 시작)
api2 = KiwoomAPI()  # 새로운 인스턴스, _ohlcv_cache = {} (비어있음)
df2 = api2.get_ohlcv("005930", 220, "20251001")  # API 호출 (캐시 없음)
```

**문제점**:
- ⚠️ **프로세스가 종료되면 메모리 캐시가 완전히 사라짐**
- TTL 1년은 프로세스 실행 중에만 의미가 있음
- 새 프로세스에서는 `_ohlcv_cache = {}`로 시작 (비어있음)
